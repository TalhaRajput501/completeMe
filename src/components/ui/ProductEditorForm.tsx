...
import { Boxes, CircleDollarSign, Delete, Donut, FileImage, PackagePlus, RotateCcw, ShieldCheck, Tag, Trash, Undo2, Warehouse } from 'lucide-react'
...
import { singleProduct, updateProduct } from '@/lib/actions/products.actions'
...
function mapValuesToOptions(values: string[], options: SelectOption[]) {
...
export default function ProductEditorForm({
  ...
  // There is a major perfornamce issue, because page on the browser is rendering... state and it is not coming in the rendered state.
  // useEffect(() => {
  //   console.log('dependency is category feature options and size options')
  //   setSelectedFeatures((prev) => prev.filter((item) => featureOptions.some((option) => option.value === item.value)))
  //   setSelectedSize((prev) => prev.filter((item) => sizeOptions.some((option) => option.value === item.value)))
  //   setProduct((prev) => ({ ...prev, category: category.value }))
  // }, [category, featureOptions, sizeOptions])
  ...
  useEffect(() => { 
    ...
    const loadProduct = async () => {
      ...
      try {
        const existingProduct = await singleProduct(productId) as ProductType | undefined
        if (!existingProduct) {
        ...
        // console.log('Loaded product for edit:', existingProduct)

        const selectedCategory = categoryOptions.find((item) => item.value === existingProduct.category) || categoryOptions[0]
        setCategory(selectedCategory)
        setLocalTags((existingProduct.tags || []).join(', '))
        setSelectedGenders(mapValuesToOptions(existingProduct.gender || [], genderOptions))
        setSelectedFeatures(mapValuesToOptions(existingProduct.features || [], featureOptionsByCategory[selectedCategory.value]))
        setSelectedSize(mapValuesToOptions(existingProduct.sizeOptions || [], sizeOptionsByCategory[selectedCategory.value]))
      } catch (loadError) {
        console.error('Failed to load product for edit', loadError)
  ...
  const handleSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    if (mode === 'edit' && selectedFiles.length === 0 && product.images && product.images.length > 0 && deletedImages.length === product.images.length) {
      setError('Please keep at least one existing image or upload a new one.')
      toast.error('Please keep at least one existing image or upload a new one.')
      return
    }
    ...
    try {
      ...
      if (!productId) {
        setError('Product ID is missing for update and no new images selected.')
        return
      }
      await updateProduct(productId, parsed.data, selectedFiles.length > 0 ? selectedFiles : undefined, deletedImages.length > 0 ? deletedImages : undefined)
      console.log('Prepared update payload (frontend only):', parsed.data, selectedFiles.length > 0 ? selectedFiles : undefined)
      ...
      const message =
        err instanceof Error
          ...
          : mode === 'create'
            ...
            : 'Something went wrong while preparing the update. Please try again.'
      setError(message)
      toast.error(message)
    }
  }
  ...
  const inputClass =
    'mt-1 h-10 w-full rounded-lg border border-slate-300 bg-white px-3 text-slate-800 placeholder:text-slate-400 focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-transparent [appearance:textfield] [&::-webkit-outer-spin-button]:appearance-none [&::-webkit-inner-spin-button]:appearance-none  '
    ...
    <div className="min-h-screen bg-gradient-to-br from-slate-50 to-blue-50 p-4 md:p-6">
      <div className="mb-6">
        ...
        <p className="text-slate-600">
          {mode === 'create'
            ...
            : 'Update product details with the same catalog form experience.'}
        </p>
      </div>
      ...
      <form onSubmit={handleSubmit} className="grid grid-cols-1 xl:grid-cols-3 gap-6">
        <div className="xl:col-span-2 space-y-6">
          ...
          {/* Existing Product Media section and Delete Section */}
          {
            mode === 'edit' && (
              <section className={sectionClass}>
                <h2 className="text-lg font-bold text-slate-800 mb-4 flex items-center gap-2">
                ...
                {/* Here you can delete existing images */}
                <div className='flex gap-x-2'>
                  {product.images && product.images.length > 0 ? (
                      ...
                      <div key={index} className="mt-2 flex items-center gap-3 ">
                        <div className="relative group">
                          ...
                          {!deletedImages.includes(img) ? (
                            <div className="absolute inset-0 bg-red-500/50 rounded flex items-center justify-center opacity-0 group-hover:opacity-100 transition-opacity">
                              <Trash onClick={() => setDeletedImages(prev => [...prev, img])} className="h-5 w-5 text-white cursor-pointer" />
                            </div>
                          ) : (
                            <div className="absolute inset-0 bg-green-500/50 rounded flex items-center justify-center opacity-0 group-hover:opacity-100 transition-opacity">
                              <RotateCcw onClick={() => setDeletedImages(prev => prev.filter((i) => i !== img))} className="h-5 w-5 text-white cursor-pointer" />
                            </div>
                          )
                          }
                        </div>
                    ...
                    <p className="mt-2 text-sm text-slate-500">There is an issue for loading images.</p>
                  )}
                </div>
              </section>
            )
          ...
          <section className={sectionClass}>
            ...
            <p className="mt-2 text-sm text-slate-500">
              {selectedFiles.length > 0
                ...
                : mode === 'edit'
                  ? 'Optional: select new image(s) for update.'
                  : 'Select one or more product images'}
            </p>
          </section>

          {/* Product Attributes section: gender, feature, size */}
          <section className={sectionClass}>
            <h2 className="text-lg font-bold text-slate-800 mb-4 flex items-center gap-2">
          ...
          {/* Description and SEO Tags section */}
          <section className={sectionClass}>
            <h2 className="text-lg font-bold text-slate-800 mb-4 flex items-center gap-2">
          ...
          <section className="rounded-xl border border-slate-200 bg-white p-4 sm:p-5 xl:sticky xl:top-6">
            ...
            <div className="mt-4 rounded-lg border border-blue-100 bg-blue-50 p-3 text-sm text-blue-700">
              ...
              <div className="mt-4 rounded-lg border border-blue-200 bg-white p-3">
                ...
                <button
                  type="submit"
                  disabled={submitting || isLoadingProduct}
